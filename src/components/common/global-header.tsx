...
// GlobalHeader(v1)와 GlobalHeaderV2 가 함께 쓴다. ref 는 헤더 전체에 단다.
...
// 2depth 줄. className 으로 왼쪽 여백과 글자색을 정한다. 높이 41 = 위아래 12 + 글자 16 + 테두리 1.
    ...
    >
      <div className="overflow-hidden">
        <ul
          ...
          className={`flex gap-[24px] border-b border-erp-bar-line py-[12px] text-[13.5px] leading-[16px] transition-opacity ease-out ${className} ${
          ...
          {menus.map((m, i) => (
            <button
              ...
              className={`flex h-[52px] shrink-0 items-center text-[16px] font-semibold whitespace-nowrap transition-colors duration-150 ease-out hover:text-erp-brand ${
                i === 0 ? "pr-[20px]" : "px-[20px]"
              } ${state.isOpen(i) ? "text-erp-brand" : "text-erp-ink"}`}
            >
              {m.label}
            </button>
          ))}
